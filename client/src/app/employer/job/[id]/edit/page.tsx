...
const editJobSchema = z.object({
  ...
  salaryFrom: z.string().min(1, "Salary from is required"),
  salaryTo: z.string().min(1, "Salary to is required"),
});
...
export default function EditJobPage() {
  ...
  useEffect(() => {
    const fetchJob = async () => {
      try {
        ...
        reset({
          ...
          salaryFrom: String(job.salaryFrom || job.fixedSalary || ""),
          salaryTo: String(job.salaryTo || job.fixedSalary || ""),
        });
      } catch (err) {
    ...
    fetchJob();
  }, [id, getToken, reset, router]);

  const onSubmit = async (data: EditJobValues) => {
    setIsLoading(true);
    try {
      const token = await getToken();
      await api.put(`/job/update/${id}`, {
        ...data,
      ...
      const message = axios.isAxiosError<{ message?: string }>(error)
        ...
        : undefined;
      alert(message || "Failed to update job");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (isFetching) {
    return (
      <div className="container mx-auto px-4 max-w-3xl py-8 min-h-screen flex items-center justify-center">
      ...
      >
        <div className="flex items-center mb-8 pb-6 border-b">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">Update the details for this job posting.</p>
          </div>
        </div>
